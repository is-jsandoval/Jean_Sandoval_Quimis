...
export const consultaLInaturales = `
 
select  sr.NUMERO_RUC as co_ruc from BG_TComercios.Pagos.SRI_ESTABLECIMIENTOS_BG SR
...
export const consultaLIjuridicos = `
...
export const consultarCtaAHOXced = `
SELECT 
    MSCTA AS CUENTA,
    'A' AS TIPO
...
export const consultarCtaCTEXced = `
SELECT 
    MTCUENTA AS CUENTA,
    'C' AS TIPO
...
--ruc para opciones AE
select top 1 
    sr.NUMERO_RUC as co_ruc
from BG_TComercios.Pagos.SRI_ESTABLECIMIENTOS_BG SR

left join nts_tarjcred.dbo.tc_comercios co 
    on sr.NUMERO_RUC = co.co_ruc

left join BG_TComercios.Pagos.lp_comercios lp 
    on co.co_comercio = lp.co_comercio_syscard
...
export const resetXcodComercio = `

SET NOCOUNT ON;

IF OBJECT_ID (N'tempdb.dbo.#comerciosLista', N'U') IS NOT NULL
DROP TABLE #comerciosLista
...
PRINT 'UPDATE tc_comercios'

update nts_tarjcred.dbo.tc_comercios
set co_linkPagos = null
where co_comercio in (
    select co_comercio from #comerciosLista
)

-----------------------------------------------------------------------

PRINT 'DELETE lp_links_logs'

delete BG_TComercios.Pagos.lp_links_logs
where lg_ruc in (
    select co_ruc from #comerciosLista
)

-----------------------------------------------------------------------

PRINT 'DELETE lp_links'

delete BG_TComercios.Pagos.lp_links
where ln_ruc in (
    select co_ruc from #comerciosLista
)
...
PRINT 'DELETE lp_consumos_link_estatico'

delete BG_TComercios.Pagos.lp_consumos_link_estatico
where ce_codigo_link_estatico in (
    select le_codigoLink
    from BG_TComercios.Pagos.lp_comercios_link_estaticos
    where le_ruc in (
        select co_ruc from #comerciosLista
    )
)
...
PRINT 'DELETE lp_comercios_link_estaticos'

delete BG_TComercios.Pagos.lp_comercios_link_estaticos
where le_ruc in (
    select co_ruc from #comerciosLista
)
...
PRINT 'DELETE lp_limites'

delete BG_TComercios.Pagos.lp_limites
where lt_comercio in (
    select co_comercio from #comerciosLista
)

-----------------------------------------------------------------------

PRINT 'DELETE lp_comercios'

delete BG_TComercios.Pagos.lp_comercios
where co_ruc in (
    select co_ruc from #comerciosLista
)
...
PRINT 'DELETE cl_telefonos'

delete nts_clientes.dbo.cl_telefonos
where tf_codigo in (
    select co_comercio from #comerciosLista
)
...
PRINT 'DELETE tc_cfechapagos'

Delete nts_tarjcred.dbo.tc_cfechapagos
where fp_comercio in (
    select co_comercio from #comerciosLista
)

-----------------------------------------------------------------------

PRINT 'DELETE tc_comercios_alternos'

Delete nts_tarjcred.dbo.tc_comercios_alternos
where ca_comercio in (
    select co_comercio from #comerciosLista
)

-----------------------------------------------------------------------

PRINT 'DELETE tc_comercio_marca'

Delete nts_tarjcred.dbo.tc_comercio_marca
where cm_comercio in (
    select co_comercio from #comerciosLista
)

-----------------------------------------------------------------------

PRINT 'DELETE tc_cafiliaciones'

Delete nts_tarjcred.dbo.tc_cafiliaciones
Where ca_comercio in (
    select co_comercio from #comerciosLista
)
...
PRINT 'DELETE tc_cliquidadepositados'

Delete nts_tarjcred.dbo.tc_cliquidadepositados
Where rd_comercio in (
    select co_comercio from #comerciosLista
)

-----------------------------------------------------------------------

PRINT 'DELETE tc_cliquidahistoria'

Delete nts_tarjcred.dbo.tc_cliquidahistoria
Where lh_comercio in (
    select co_comercio from #comerciosLista
)

-----------------------------------------------------------------------

PRINT 'DELETE tc_cliquidadiaria'

Delete nts_tarjcred.dbo.tc_cliquidadiaria
Where ld_comercio in (
    select co_comercio from #comerciosLista
)

-----------------------------------------------------------------------

PRINT 'DELETE tc_tcomerdiferi'

Delete nts_tarjcred.dbo.tc_tcomerdiferi
Where cd_comercio in (
    select co_comercio from #comerciosLista
)

-----------------------------------------------------------------------

PRINT 'DELETE tc_comercioslimitepiso'

Delete nts_tarjcred.dbo.tc_comercioslimitepiso
Where lp_comercio in (
    select co_comercio from #comerciosLista
)

-----------------------------------------------------------------------

PRINT 'DELETE tc_tdiferidos'

Delete nts_tarjcred.dbo.tc_tdiferidos
Where di_comercio in (
    select co_comercio from #comerciosLista
)
...
PRINT 'DELETE tc_transacciones_hist'

WHILE 1 = 1
BEGIN

    DELETE TOP (1000)
    FROM nts_tarjcred.dbo.tc_transacciones_hist
    WHERE th_comercio in (
        select co_comercio from #comerciosLista
    )
...
PRINT 'DELETE tc_transacciones'

WHILE 1 = 1
BEGIN

    DELETE TOP (1000)
    FROM nts_tarjcred.dbo.tc_transacciones
    WHERE tn_comercio in (
        select co_comercio from #comerciosLista
    )
...
PRINT 'DELETE tc_comercio_empresa'

Delete nts_tarjcred.dbo.tc_comercio_empresa
where ce_comercio in (
    select co_comercio from #comerciosLista
)

-----------------------------------------------------------------------

PRINT 'DELETE tc_comercios'

Delete nts_tarjcred.dbo.tc_comercios
where co_comercio in (
    select co_comercio from #comerciosLista
)
...
export const verificarComercioLinkCreado = `
SELECT 
    CASE 
        WHEN EXISTS (
            SELECT 1
...
export const activarGiro = `update BG_TComercios.Pagos.SRI_ESTABLECIMIENTOS_BG set CODIGO_CIIU = 'C264001' where NUMERO_RUC =@ruc`